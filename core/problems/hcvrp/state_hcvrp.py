...
class StateHCVRP(NamedTuple):
    ...
    VEHICLE_CAPACITY = [20., 25., 30.]  # Hardcoded
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        depot = input['depot'] 
        loc = input['loc'] # add time window (x, y)
        demand = input['demand'] 
        time_window = input['time_window']
        # [0, 0, 0, 0, 1, 1, 1, 0]
        """
        1. adapt with time window
        2. input architecture
        3. cost also with time window
        4. vehicle arrive time window = 0, cost high/reward low
        """
    ...
    def get_mask(self, veh):
        ...
        cap = torch.tensor(
            ...
            device=self.used_capacity.device,
            dtype=self.used_capacity.dtype,
        )[None, :].expand(batch_size, len(self.VEHICLE_CAPACITY))
        cap_at_veh = (cap[self.ids, veh][0].unsqueeze(-1))[..., None].expand_as(self.demand[self.ids, 1:])
        demand_here = self.demand[self.ids, 1:]
        used = self.used_capacity[torch.arange(batch_size, device=self.used_capacity.device), veh].unsqueeze(-1)
        used = used[..., None].expand_as(demand_here)
        exceeds_cap = (demand_here + used > cap_at_veh)
