...
def rollout(model, dataset, opts):
    ...
    def eval_model_bat(bat):
        ...
        with torch.no_grad():
            out = model(move_to(bat, opts.device))
            cost = out[0]
...
def train_batch(
    ...
    if len(out) == 4 and isinstance(out[3], torch.Tensor) and out[3].dim() == 3 and out[3].size(0) == 4:
        cost, log_likelihood, log_veh, pack = out
        costs_p, ll_p, llv_p = pack[0], pack[1], pack[2]
        bl_val, bl_loss = baseline.eval(x, cost) if bl_val is None else (bl_val, 0)
        reinforce_loss = ((costs_p - bl_val) * (ll_p + llv_p)).mean()
        kl_term = -float(getattr(opts, 'kl_loss', 0) or 0) * pack[3].mean()
