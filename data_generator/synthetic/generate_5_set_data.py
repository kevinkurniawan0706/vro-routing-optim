...
def generate_hcvrp_data(dataset_size, hcvrp_size, veh_num):
    ...
    for instance_index in range(dataset_size * 10):  # Total 1280 instances
        seed = base_seed + instance_index
        rnd = np.random.RandomState(seed)

        loc = rnd.uniform(0, 1, size=(1, hcvrp_size + 1, 2))
        depot = loc[:, -1]
        cust = loc[:, :-1]
        d = rnd.randint(1, 10, [1, hcvrp_size + 1])
        d = d[:, :-1]  # the demand of depot is 0, which do not need to generate here

        ## Start time window
        # Initialize time windows with zeros
        time_windows = np.zeros((1, hcvrp_size, 24))
...
if __name__ == "__main__":
    ...
    # Use a random seed for generating different `.pkl` files
    for i in range(5):
        seed = random.randint(10000, 99999)  # Random seed for overall dataset
        np.random.seed(seed)
        print(opts.dataset_size, opts.graph_size)
        filename = os.path.join(datadir, '{}_v{}_{}_set{}_seed{}_tw.pkl'.format(problem, opts.veh_num, opts.graph_size, i+1, seed))
